...
class BarkleyTracker:
    ...
    def process_frame(self, frame, pose_roi_x=100, pose_roi_y=100, book_roi_x=100, book_roi_y=100, pose_roi_size=1080, book_roi_size=224):
        ...
        # 只針對 "sitting" 和 "not sitting" 做平滑，其他狀態直接使用
        status_smoothed = None
        if status_list:
            # 篩選出有效的坐姿判定（不是其他信心度不足的狀態）
            ...
            if valid_statuses:
                ...
                status_text = status_smoothed
                if status_smoothed != valid_statuses[0] if valid_statuses else False:
                    status_text += f" (raw: {valid_statuses[0]})"
            else:
                # 所有人都是信心度不足的狀態，直接用第一個人的狀態
                status_text = status_list[0] if status_list else "no person"
        else:
        ...
        cv2.putText(frame, f"Pose ROI ({num_persons} persons) | {status_text}", (pose_roi_x + 10, pose_roi_y + 25),
...
def save_pose_json(keypoints_data, output_dir=None, label=None, roi_image=None):
    ...
    for i, person in enumerate(persons_with_debug):
        pred_status = person["sitting_detection"]["status"]
        ...
        if label:
            mark = "OK" if pred_status in ["sitting", "not sitting"] and ((pred_status == "sitting") == (label == "sitting")) else "MISMATCH!"
            print(f"  Person {i}: label={label}, predicted={pred_status} [{mark}] ({feat_str})")
        else:
            print(f"  Person {i}: predicted={pred_status} ({feat_str})")
